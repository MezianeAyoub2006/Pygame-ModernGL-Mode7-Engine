...
def pixels_to_ndc(ox, oy, sw, sh, W, H):
    x1 = (ox / W) * 2 - 1
    y1 = 1 - ((oy + sh) / H) * 2
    x2 = ((ox + sw) / W) * 2 - 1
    y2 = 1 - (oy / H) * 2
...
class ShaderPass:
    """ 
    Interface that handles modernGl textures, their loading and rendering, for 2D puproses (no vertex shader)
    """
    ...
    def set_viewport(self, ox, oy, sw, sh):
    ...
    def load_texture(self, name, path, slot, filter=moderngl.NEAREST):
